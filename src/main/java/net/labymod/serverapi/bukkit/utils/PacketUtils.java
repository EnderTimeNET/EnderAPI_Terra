...
public class PacketUtils {
        ...
        if ( this.packetPlayOutCustomPayloadClass != null ) {
            ...
            if ( !customPayloadHasBytes ) {
                try {
                    ...
                    ex.printStackTrace();
                    EnderAPI.getInstance().getPlugin().getLogger().severe( "Couldn't find a valid constructor for PacketPlayOutCustomPayload. Disabling the plugin." );
                    Bukkit.getPluginManager().disablePlugin( EnderAPI.getInstance().getPlugin() );
                }
            }
        }
    }
    ...
    public Object getPluginMessagePacket( String channel, byte[] bytes ) {
        try {
            ...
            EnderAPI.getInstance().getPlugin().getLogger().severe( "Couldn't construct a custom-payload packet (Channel: " + channel + "):" );
            e.printStackTrace();
